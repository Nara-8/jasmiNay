...
Proof.
  apply: (cmd_rect (Pr := Pr) (Pi := Pi) (Pc := Pc)) => /= {c s}
    [ i ii Hi | | i c Hi Hc | x tg ty e | xs t o es | p x e | t p e | e c1 c2 Hc1 Hc2
...
Proof.
  ...
  split => //=
    ...
    | idx He op x body He1 start He2 len He3 | x | v e1 e2 He1 He2 | e1 e2 He1 He2] s;
...
Lemma read_e_Pbig idx op x body s len :
  Sv.Equal (read_e (Pbig idx op x body s len))
           (Sv.union (read_e idx) (Sv.union (Sv.remove x (read_e body))
              (Sv.union (read_e s) (read_e len)))).
Proof. rewrite {1}/read_e /= !read_eE; clear; SvD.fsetdec. Qed.
...
Lemma read_i_assert t p e :
  Sv.Equal (read_i (Cassert t p e)) (read_e e).
Proof.
...
Lemma vars_I_assert ii t p e:
  Sv.Equal (vars_I (MkI ii (Cassert t p e))) (read_e e).
Proof.
...
Lemma eq_var_trans (x2 x1 x3: var) : x1 == x2 → x2 == x3 → x1 == x3.
Proof. by move => /eqP -> /eqP ->; rewrite !eqxx. Qed.
...
Proof.
  ...
  + move=> o es1 hrec [] //= ? es2 [] ? es3 //=.
    by move=> /andP[]/eqP-> h1 /andP[]/eqP-> h2;rewrite eqxx /=; eauto.
