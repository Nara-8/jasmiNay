...
Fixpoint dead_code_i (i:instr) (s:Sv.t) {struct i} : cexec (Sv.t * option instr) :=
  ...
  | Cassgn x tag ty e =>
    let w := write_i ir in
    if tag != AT_keep then
      if (disjoint s w && negb (lv_write_mem x)) then ok (s, None)
  ...
  | Cwhile a c e ei c' =>
    ...
    let: (s, (c,c')) := sc in
    ok (s, Some (MkI ii (Cwhile a c e ei c')))
...
Definition dead_code_prog_tokeep (p: prog) : cexec prog :=
  ...
  ok {| p_extra := p_extra p; p_abstr := p_abstr p; p_globs := p_globs p; p_funcs := funcs |}.

End Section.

End ONFUN.

Definition dead_code_prog {pT:progT} (p:prog) (do_nop: bool) :=
  @dead_code_prog_tokeep (fun _ => None) pT p.
